...
class PcmEdgeEngine(BaseEngine):
    """
    A lightweight Edge TTS engine that decodes MP3 output to raw PCM 16-bit
    before queueing. This avoids the crackling/distortion caused by the
    built-in EdgeEngine which queues raw MP3 frames as if they were PCM.

    NOTE: We do NOT call super().__init__() directly here because BaseInitMeta
    calls BaseEngine.__init__ automatically after __init__ runs. Setting
    instance attributes in post_init() ensures they survive that second call.
    """
    def __init__(self, voice: str = "vi-VN-HoaiMyNeural"):
        self._init_voice = voice

    def post_init(self):
        """Called by BaseInitMeta after BaseEngine.__init__ has run."""
        self.engine_name = "edge"
        self.voice = self._init_voice
        # BaseEngine.__init__ already set self.queue = queue.Queue(); we keep it.

    def get_stream_info(self):
        import pyaudio
        # Return standard 16-bit mono at 24000 Hz (Edge TTS native rate)
        return pyaudio.paInt16, 1, 24000

    def synthesize(self, text: str) -> bool:
        """
        Fetches audio via edge-tts, decodes MP3 → PCM 16-bit @ 24000 Hz,
        and puts fixed-size chunks into self.queue.
        """
        from edge_tts import Communicate
        import re

        # Clean text from markdown formatting (bold, headers, lists, backticks)
        clean_text = text
        ...
        # Remove list item indicators (- text, * text, + text at the beginning of a line/sentence)
...
class AudioProcessor:
    ...
    def __init__(
        ...
        if engine == "coqui":
            ...
            self.engine = PcmEdgeEngine(voice="vi-VN-HoaiMyNeural")
        else:
        ...
        # PcmEdgeEngine already returns correct PCM 16-bit@24000Hz — no override needed
