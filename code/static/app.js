...
function handleJSONMessage({ type, content, is_correction }) {
  ...
  if (type === "final_user_request") {
    if (content?.trim()) {
      if (is_correction && chatHistory.length > 0 && chatHistory[chatHistory.length - 1].role === "user") {
  ...
  if (type === "ai_finished_speaking") {
    // This message indicates the AI has finished speaking its final answer
    // The client-side doesn't need to do anything immediately, the server handles the timer.
    console.log("AI finished speaking. Server-side timer for suggestions initiated.");
    return;
  }
}
