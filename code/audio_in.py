...
class AudioInputProcessor:
    ...
    _RESAMPLE_RATIO = 1  # Input is now 16kHz directly from browser
    ...
    def __init__(
        ...
        self._interrupted_at: float = 0.0  # Timestamp when interrupted was set
        self._INTERRUPT_SAFETY_TIMEOUT: float = 5.0  # Safety timeout to force-reset
    ...
    def interrupted(self) -> bool:
        """Whether audio input is currently interrupted (e.g., during TTS playback transition)."""
    ...
    def interrupted(self, value: bool) -> None:
        if value and not self._interrupted:
            ...
            logger.info("👂⏸️ Audio input interrupted.")
        elif not value and self._interrupted:
            duration = time.time() - self._interrupted_at if self._interrupted_at else 0
            logger.info(f"👂▶️ Audio input interrupt cleared (was interrupted for {duration:.2f}s).")
    ...
    async def process_chunk_queue(self, audio_queue: asyncio.Queue) -> None:
        ...
        while True:
            try:
                ...
                # Feed audio to the transcriber, but substitute silence when interrupted
                # to keep the recorder's VAD/state machine running (so it can properly
                # detect silence end and transition out of "recording" mode).
                if not self._transcription_failed:
                    if self.interrupted:
                        # Feed silence so the recorder's VAD detects silence and
                        # properly ends the current recording instead of getting stuck
                        silent_chunk = np.zeros_like(processed).tobytes()
                        self.transcriber.feed_audio(silent_chunk, audio_data)
                    else:
