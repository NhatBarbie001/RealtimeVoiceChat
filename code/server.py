...
# Optimize PyTorch CPU usage to avoid thread contention on CPU-only laptops
try:
    import torch
    torch.set_num_threads(4)
    # Set environment variables for openmp/mkl thread limits
...
# Default speech input language for transcription.
# Set to "vi" so the app listens for Vietnamese by default.
LANGUAGE = "vi"
    ...
    try:
        while True:
            ...
            if msg_type == "close_connection":
                logger.info(Colors.apply("🖥️🔌 Closing WebSocket connection due to inactivity.").orange)
                await ws.close(code=1000, reason=data.get("content", "Inactivity timeout"))
                break
...
async def send_tts_chunks(app: FastAPI, message_queue: asyncio.Queue, callbacks: 'TranscriptionCallbacks') -> None:
    ...
    try:
        ...
        while True:
            ...
            chunk = None
            try:
                current_gen = app.state.SpeechPipelineManager.running_generation
                chunk = current_gen.audio_chunks.get_nowait() if current_gen else None
                if chunk:
                    last_quick_answer_chunk = time.time()
                    if current_gen and not getattr(current_gen, "first_chunk_sent_time", 0.0):
                        current_gen.first_chunk_sent_time = last_quick_answer_chunk
                        
                        start = getattr(current_gen, "start_time", 0.0)
                        llm_start = getattr(current_gen, "llm_start_time", 0.0)
                        llm_ft = getattr(current_gen, "llm_first_token_time", 0.0)
                        llm_qa = getattr(current_gen, "llm_quick_answer_time", 0.0)
                        tts_start = getattr(current_gen, "tts_quick_start_time", 0.0)
                        tts_ft = getattr(current_gen, "tts_first_audio_chunk_time", 0.0)
                        sent = current_gen.first_chunk_sent_time
                        
                        logger.info(Colors.apply("\n⏱️⚡ [LATENCY BREAKDOWN] --------------------").green.bold)
                        if start and llm_start:
                            logger.info(f"  • Pipeline Start to LLM Call: {(llm_start - start)*1000:.2f} ms")
                        if llm_start and llm_ft:
                            logger.info(f"  • LLM TTFT (Time to First Token): {(llm_ft - llm_start)*1000:.2f} ms")
                        if llm_ft and llm_qa:
                            logger.info(f"  • LLM Quick Answer Ready: {(llm_qa - llm_ft)*1000:.2f} ms")
                        if tts_start and tts_ft:
                            logger.info(f"  • TTS Quick Start to First Audio Chunk: {(tts_ft - tts_start)*1000:.2f} ms")
                        if tts_ft and sent:
                            logger.info(f"  • First Audio Chunk to Server Sent: {(sent - tts_ft)*1000:.2f} ms")
                        if start and sent:
                            logger.info(f"  • {Colors.apply('TOTAL RESPONSE LATENCY:').cyan.bold} {(sent - start)*1000:.2f} ms")
                        logger.info(Colors.apply("-------------------------------------------").green.bold)
            except Empty:
                current_gen = app.state.SpeechPipelineManager.running_generation
                if current_gen:
                    final_expected = current_gen.quick_answer_provided
                    audio_final_finished = current_gen.audio_final_finished
                    ...
                    if not final_expected or audio_final_finished:
                        ...
                        # Print generation summary before resetting
                        start = current_gen.start_time
                        llm_finish = current_gen.llm_finished_time if current_gen.llm_finished_time else time.time()
                        tts_quick_fin = current_gen.tts_quick_finished_time
                        tts_final_fin = current_gen.tts_final_finished_time
                        
                        logger.info(Colors.apply("\n📊📊 [GENERATION SUMMARY] ------------------").blue.bold)
                        if start and llm_finish:
                            logger.info(f"  • Total LLM Generation Time: {(llm_finish - start)*1000:.2f} ms")
                        if current_gen.tts_quick_start_time and tts_quick_fin:
                            logger.info(f"  • Quick TTS Duration: {(tts_quick_fin - current_gen.tts_quick_start_time)*1000:.2f} ms")
                        if current_gen.tts_final_start_time and tts_final_fin:
                            logger.info(f"  • Final TTS Duration: {(tts_final_fin - current_gen.tts_final_start_time)*1000:.2f} ms")
                        logger.info(Colors.apply("---------------------------------------------").blue.bold)

                        callbacks.send_final_assistant_answer() # Callbacks method

                        assistant_answer = current_gen.quick_answer + current_gen.final_answer                    
...
class TranscriptionCallbacks:
    ...
    """
    def __init__(self, app: FastAPI, message_queue: asyncio.Queue, loop: asyncio.AbstractEventLoop):
        """
        ...
        Args:
            ...
            loop: The main event loop for thread-safe operations.
        """
        self.app = app
    ...
    def send_to_client(self, data: dict):
        """Sends a JSON message to the client thread-safely."""
        self.loop.call_soon_threadsafe(self.message_queue.put_nowait, data)

    async def _start_suggestion_timer(self):
        """
        Starts an asyncio task to wait for a period and then trigger a suggestion
        if the user hasn\"t responded.
        """
        self.cancel_suggestion_timer() # Ensure any previous timer is cancelled
        logger.info(f"🖥️⏱️ Starting suggestion timer for {self.SUGGESTION_DELAY_SECONDS} seconds.")
        self.suggestion_timer_task = asyncio.create_task(self._wait_and_suggest())

    def cancel_suggestion_timer(self):
        """
        Cancels the active suggestion timer if one is running.
        """
        if self.suggestion_timer_task and not self.suggestion_timer_task.done():
            self.loop.call_soon_threadsafe(self.suggestion_timer_task.cancel)
            logger.info("🖥️⏱️ Suggestion timer cancelled thread-safely.")
            self.suggestion_timer_task = None

    async def _wait_and_suggest(self):
        """
        Waits for a defined period and then requests a suggestion from the LLM
        if no user input has been received in the interim.
        """
        try:
            await asyncio.sleep(self.SUGGESTION_DELAY_SECONDS)
               ...
               (time.time() - self.ai_finished_speaking_time) >= self.SUGGESTION_DELAY_SECONDS:
                ...
                if self.consecutive_suggestions_count <= 2:
                    ...
                    self.send_to_client({
                        ...
                        "content": "Conversation ended due to user inactivity."
                    })

                self.suggestion_timer_task = None # Task is complete

        except asyncio.CancelledError:
            logger.info("\ud83d\udda5\ufe0f\u23f1\ufe0f Suggestion timer task was cancelled.")
        except Exception as e:
            logger.error(f"\ud83d\udda5\ufe0f\ud83d\udca5 Error in suggestion timer task: {e}", exc_info=True)
    ...
    def map_to_closest_bidv_question(self, txt: Any) -> str:
        ...
        similarity_checker = TextSimilarity(focus='overall')
        
        best_match = BIDV_SAVINGS_QUESTIONS[0]
        best_score = -1.0
        
        for question in BIDV_SAVINGS_QUESTIONS:
            score = similarity_checker.calculate_similarity(trimmed, question)
            if score > best_score:
                best_score = score
                best_match = question
        if score < 0.1:
    ...
    def on_partial(self, txt: str):
        ...
        """
        mapped_txt = self.map_to_closest_bidv_question(txt)
        if not mapped_txt:
            return
        ...
        self.partial_transcription = mapped_txt
        self.send_to_client({"type": "partial_user_request", "content": mapped_txt})
        self.abort_text = mapped_txt # Update text used for abort check
    ...
    def on_potential_sentence(self, txt: str):
        ...
        if not mapped_txt:
            return
        logger.debug(f"🖥️🧠 Potential sentence (mapped): '{mapped_txt}'")
        # Access global manager state
        self.app.state.SpeechPipelineManager.prepare_generation(mapped_txt)
    ...
    def on_potential_final(self, txt: str):
        ...
        if not mapped_txt:
            return
        logger.info(f"{Colors.apply('🖥️🧠 HOT: ').magenta}{mapped_txt}")
    ...
    def on_before_final(self, audio: bytes, txt: str):
        ...
        user_request_content = mapped_txt if mapped_txt else (self.final_transcription if self.final_transcription else self.partial_transcription)
        self.send_to_client({
    ...
    def on_final(self, txt: str):
        ...
        Logs the final transcription and stores it. Starts or corrects LLM generation
        if the final text is different from what was prepared during turn-end detection.
        ...
        if not mapped_txt:
            return
        self.final_transcription = mapped_txt

        # Check if we need to start/correct the LLM generation with this final text
        manager = self.app.state.SpeechPipelineManager
        current_gen = manager.running_generation

        is_already_generating_correct_text = False
        if current_gen and current_gen.text:
            from text_similarity import TextSimilarity
            similarity = TextSimilarity(focus='end', n_words=5).calculate_similarity(current_gen.text, mapped_txt)
            if similarity >= 0.95:
                is_already_generating_correct_text = True

        if not is_already_generating_correct_text:
            logger.info(Colors.apply(f"🖥️🧠 Triggering/Correcting generation for final text: '{mapped_txt}'").green.bold)
            
            # Correct the last user request in history if it exists and matches
            if manager.history and manager.history[-1]["role"] == "user":
                manager.history[-1]["content"] = mapped_txt
            else:
                manager.history.append({"role": "user", "content": mapped_txt})

            # Send the updated final user request to the client
            self.send_to_client({
                "type": "final_user_request",
                "content": mapped_txt,
                "is_correction": True
            ...
            # Prepare the new/corrected generation
            manager.prepare_generation(mapped_txt)

            # Ensure TTS is released to the client
            self.tts_to_client = True
    ...
    def on_recording_start(self):
        ...
        ai_is_active = (self.tts_client_playing or self.tts_to_client or self.tts_chunk_sent)
        if ai_is_active:
            ...
            # Run abort in a separate thread so we don't block the recorder's callback thread
            threading.Thread(
                target=self.abort_generations,
                args=("on_recording_start, user interrupts, TTS Playing",),
                daemon=True
            ).start()

            logger.info("🖥️❗ Sending tts_interruption to client.")
            self.send_to_client({ # Tell client to stop playback and clear buffer
    ...
    def send_final_assistant_answer(self, forced=False):
        ...
        if not self.final_assistant_answer_sent and final_answer:
            ...
            if cleaned_answer: # Ensure it\'s not empty after cleaning
                ...
                if not getattr(self, "is_saying_goodbye", False):
                    self.on_ai_finished_speaking()
                self.send_to_client({
                    "type": "ai_finished_speaking",
                    "content": ""
                }) # Signal client that AI has finished speaking
            else:
    ...
    def on_ai_finished_speaking(self):
        """
        Method to be invoked when the AI finishes speaking to start the suggestion timer.
        """
        self.ai_finished_speaking_time = time.time()
        asyncio.run_coroutine_threadsafe(self._start_suggestion_timer(), self.loop)
...
async def websocket_endpoint(ws: WebSocket):
    ...
    callbacks = TranscriptionCallbacks(app, message_queue, loop)
    ...
    # No need to assign on_ai_finished_speaking here, as it's called directly from SpeechPipelineManager
    ...
    # --- AI greeting: speak first after a short delay for client to initialize ---
    async def _trigger_greeting():
        """Wait for client to finish mic/TTS setup, then send the opening greeting."""
        from starlette.websockets import WebSocketState
        await asyncio.sleep(1.5)  # Allow client audio worklets to initialize
        if ws.client_state == WebSocketState.CONNECTED:
            logger.info("🖥️👋 Triggering AI opening greeting...")
            callbacks.tts_to_client = True
            await asyncio.to_thread(
                app.state.SpeechPipelineManager.process_greeting,
                message_queue
            )
