...
try:
    ...
    system_prompt = "Bạn là một trợ lý tiếng Việt hữu ích."
...
class RunningGeneration:
    ...
    def __init__(self, id: int):
        ...
        self.llm_quick_answer_time: float = 0.0
        self.llm_finished_time: float = 0.0
        self.tts_quick_start_time: float = 0.0
        self.tts_first_audio_chunk_time: float = 0.0
        self.tts_quick_finished_time: float = 0.0
        self.tts_final_start_time: float = 0.0
        self.tts_final_finished_time: float = 0.0
        self.first_chunk_sent_time: float = 0.0
...
class SpeechPipelineManager:
    ...
    def __init__(
        ...
        if self.llm_inference_time is not None:
            ...
            logger.debug("🗣️🧠🕒 LLM inference time: N/A")
        ...
        self.on_ai_finished_speaking: Optional[Callable[[asyncio.Queue], None]] = None # New callback for AI finish speaking

        latency_llm = self.llm_inference_time or 0.0
        self.full_output_pipeline_latency = latency_llm + self.audio.tts_inference_time
        # Ép giá trị về 0.0 nếu biến bị None để tránh lỗi định dạng :.2f
        latency_full = self.full_output_pipeline_latency or 0.0
        latency_tts = getattr(self.audio, 'tts_inference_time', 0.0) or 0.0

        logger.info(f"🗣️⏱️ Full output pipeline latency: {latency_full:.2f}ms (LLM: {latency_llm:.2f}ms, TTS: {latency_tts:.2f}ms)")
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                for chunk in current_gen.llm_generator:
                    ...
                    if token_count == 1:
                        current_gen.llm_first_token_time = time.time()
                    ...
                    if not current_gen.quick_answer_provided:
                        ...
                        if context:
                            ...
                            current_gen.llm_quick_answer_time = time.time()
                ...
                current_gen.llm_finished_time = time.time()
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                ...
                if self.stop_tts_quick_request_event.is_set() or current_gen.abortion_started:
                    ...
                    current_gen.tts_quick_start_time = time.time()
                ...
                current_gen.tts_quick_finished_time = time.time()
        ...
        *same* `audio_chunks` queue used by the quick worker. Handles stop requests
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                ...
                current_gen.tts_final_start_time = time.time()
                ...
                current_gen.tts_final_finished_time = time.time()
                ...
                # After assistant finishes speaking, trigger the callback in server.py
                if self.on_ai_finished_speaking:
                    self.on_ai_finished_speaking(current_gen.audio_chunks) # Pass the message queue for suggestions
    ...
    def process_greeting(self, message_queue: asyncio.Queue):
        """
        Generates and sends a greeting when a new client connects.
        Uses LLM to generate a polite, contextual greeting in Vietnamese.
        """
        logger.info("🗣️👋 Generating opening greeting for new client session.")
        try:
            greeting_prompt = (
                "Bạn là một trợ lý AI thông minh và thân thiện. Hãy chào hỏi lịch sự, "
                "ngắn gọn bằng tiếng Việt khi bắt đầu cuộc trò chuyện. "
                "Ví dụ: chào hỏi, giới thiệu bản thân và hỏi bạn có thể giúp gì. "
                "Chỉ trả về văn bản thuần túy, không dùng markdown hay giải thích thêm."
            )

            greeting_generator = self.llm.generate(
            ...
            greeting = "".join(list(greeting_generator)).strip()
            greeting = greeting.replace("```markdown", "").replace("```", "").strip()

            if not greeting:
                greeting = "Xin chào! Em là trợ lý AI, rất vui được hỗ trợ anh chị. Anh chị cần em giúp gì không ạ?"

            logger.info(f"🗣️👋 Greeting generated: '{greeting}'")

            # Inject as a pseudo-generation through the TTS pipeline
            self.generation_counter += 1
            new_gen_id = self.generation_counter
            self.running_generation = RunningGeneration(id=new_gen_id)
            self.running_generation.text = "AI Greeting"
            self.running_generation.quick_answer = greeting
            self.running_generation.quick_answer_provided = True
            self.running_generation.llm_generator = iter([])
            self.running_generation.llm_finished = True
            self.running_generation.llm_quick_answer_time = time.time()
            self.llm_answer_ready_event.set()

            if self.on_partial_assistant_text:
                self.on_partial_assistant_text(greeting)

        except Exception as e:
            logger.error(f"🗣️💥 Error generating greeting: {e}", exc_info=True)

    def process_generate_suggestion(self, message_queue: asyncio.Queue, suggestion_type: str = "normal"):
        """
        Generates a follow-up suggestion based on chat history using the LLM and sends it to the client.
        """
        if suggestion_type != "goodbye" and not self.history:
        ...
        try:
            # Construct a prompt for the LLM to generate a suggestion
            if suggestion_type == "normal":
                suggestion_prompt = "Dựa trên lịch sử trò chuyện phía trước, hãy đưa ra một câu hỏi gợi ý ngắn gọn và lôi cuốn bằng tiếng Việt để tiếp tục cuộc trò chuyện. Chỉ trả về câu hỏi gợi ý, không giải thích gì thêm."
            elif suggestion_type == "check_presence":
                suggestion_prompt = "Dựa trên lịch sử trò chuyện phía trước, người dùng đã im lặng một lúc. Hãy hỏi thăm tự nhiên xem họ còn đó không hoặc có muốn tiếp tục không bằng tiếng Việt. Chỉ trả về câu hỏi thăm ngắn gọn."
            elif suggestion_type == "goodbye":
                # Polite farewell before closing - no history needed
                suggestion_prompt = (
                    "Người dùng đã im lặng quá lâu và cuộc trò chuyện sắp kết thúc. "
                    "Hãy nói lời tạm biệt lịch sự, ngắn gọn bằng tiếng Việt, "
                    "ví dụ xin phép tắt máy nếu anh chị bận hoặc không còn ở đó. "
                    "Chỉ trả về văn bản thuần túy, không dùng markdown."
                )
            else:
            ...
            suggestion = suggestion.replace("```markdown", "").replace("```", "").strip()

            # Fallback farewell if LLM returns empty
            if not suggestion and suggestion_type == "goodbye":
                suggestion = "Nếu anh chị bận hoặc không còn ở đó, em xin phép tắt máy ạ. Hẹn gặp lại anh chị!"

            if suggestion:
                logger.info(f"🗣️💡 Generated suggestion: '{suggestion}'")

                # Create a pseudo-generation to send the suggestion via TTS pipeline
                self.generation_counter += 1
                new_gen_id = self.generation_counter
                self.running_generation = RunningGeneration(id=new_gen_id)
                self.running_generation.text = "AI Suggestion"
                self.running_generation.quick_answer = suggestion
                self.running_generation.quick_answer_provided = True
                self.running_generation.llm_generator = iter([])  # Prevent final TTS from crashing
                self.running_generation.llm_finished = True
                self.running_generation.llm_quick_answer_time = time.time()
                self.llm_answer_ready_event.set()

                # Send the partial text so the UI updates immediately
                if self.on_partial_assistant_text:
                    self.on_partial_assistant_text(suggestion)
                # The normal TTS pipeline will handle sending final_assistant_answer and appending to history

            else:
                logger.warning("🗣️💡 LLM returned an empty suggestion.")

        except Exception as e:
            logger.error(f"🗣️💥 Error generating suggestion: {e}", exc_info=True)
    ...
    def reset(self):
        ...
        self.history = []
        if hasattr(self.llm, "reset_session"):
