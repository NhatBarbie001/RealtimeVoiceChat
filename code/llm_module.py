...
AWLEE_BASE_URL = os.getenv("AWLEE_BASE_URL", "https://owlla-dev.thinklabs.com.vn")
AWLEE_USERNAME = os.getenv("AWLEE_USERNAME", "demo_nhat")
AWLEE_PASSWORD = os.getenv("AWLEE_PASSWORD", "Demo@123")
AWLEE_AGENT_ID = os.getenv("AWLEE_AGENT_ID", "6a2f77a0c290abb2ca260a67")
AWLEE_SECTION_ID = os.getenv("AWLEE_SECTION_ID", "699bcc6d089d7585f780bb296")
AWLEE_SESSION_ID = os.getenv("AWLEE_SESSION_ID", "69a11f5f3aad2f2a66a12db1")
...
class LLM:
    ...
    def __init__(
        ...
        if self.backend == "awlee" and REQUESTS_AVAILABLE:
            self.awlee_session = requests.Session()
            self._awlee_authenticated = False
            import secrets
            env_session_id = os.getenv("AWLEE_SESSION_ID")
            if env_session_id:
                self.awlee_session_id = env_session_id
            else:
                self.awlee_session_id = secrets.token_hex(12)
            logger.info(f"🤖🔌 Initialized requests.Session for Awlee backend. Session ID: {self.awlee_session_id}")
    ...
    def reset_session(self):
        """Generates a fresh session ID for the Awlee backend."""
        if self.backend == "awlee":
            import secrets
            self.awlee_session_id = secrets.token_hex(12)
            # We also need to re-authenticate or clear previous authentication to make sure it's fresh
            self._client_initialized = False
            self._awlee_authenticated = False
            logger.info(f"🤖🔄 Reset Awlee session. New Session ID: {self.awlee_session_id}")
            ...
            try:
                if self.backend == "openai":
                    ...
                    if self.ollama_session and self.effective_ollama_url:
                        ...
                        init_ok = False
                elif self.backend == "awlee":
                    if self.awlee_session:
                        try:
                            logger.info(f"🤖🔌 Logging in to Awlee at {AWLEE_BASE_URL}...")
                            response = self.awlee_session.post(
                                f"{AWLEE_BASE_URL}/api/login",
                                json={"username": AWLEE_USERNAME, "password": AWLEE_PASSWORD},
                                timeout=10.0
                            )
                            response.raise_for_status()
                            logger.info("🤖🔌✅ Successfully authenticated with Awlee.")
                            self._awlee_authenticated = True
                            init_ok = True
                        except Exception as e:
                            logger.error(f"🤖💥 Awlee login failed: {e}")
                            self._awlee_authenticated = False
                            init_ok = False
                    else:
                        logger.error("🤖💥 Awlee session object is None.")
                        init_ok = False
                ...
                if self.backend == "awlee" and not init_ok:
                    self._awlee_authenticated = False
        ...
        **kwargs: Any
        ...
        try:
            if self.backend == "openai":
                ...
                if self.awlee_session is None or not self._awlee_authenticated:
                    raise RuntimeError("Awlee session not initialized or authenticated.")
                awlee_api_url = f"{AWLEE_BASE_URL}/api/streaming-chat"
                
                # Awlee backend does not natively support a messages array.
                # Format system prompt and history into the content string to provide conversational memory and instructions.
                system_prompt_str = ""
                if use_system_prompt and self.system_prompt:
                    system_prompt_str = f"Chỉ dẫn hệ thống:\n{self.system_prompt}\n\n"
                ...
                if history:
                    ...
                    history_str += f"\nUser: {text}\nHãy trả lời bằng tiếng Việt dựa trên ngữ cảnh trên. Trả lời trực tiếp, không lặp lại lịch sử.\nAI:"
                    formatted_content = system_prompt_str + history_str
                else:
                    if system_prompt_str:
                        formatted_content = system_prompt_str + f"User: {text}\nAI:"
                    
                payload = {
                    "agent_id": AWLEE_AGENT_ID,
                    "content": formatted_content,
                    "conversation_type": "platform",
                    "conversation_id": None,
                    "section_id": AWLEE_SECTION_ID,
                    "session_id": self.awlee_session_id,
                    "attachments": [],
                    "type_message": "text",
                }
                logger.info(f"🤖💬 [{req_id}] Sending Awlee request to {awlee_api_url} with payload:")
                logger.info(f"{json.dumps(payload, indent=2)}")
                response = self.awlee_session.post(
                    awlee_api_url, json=payload, stream=True, timeout=(10.0, 600.0)
                )
                response.raise_for_status()
                stream_object_to_register = response
                self._register_request(req_id, "awlee", stream_object_to_register)
                yield from self._yield_awlee_chunks(response, req_id)
            else:
    ...
    def _yield_awlee_chunks(self, response: requests.Response, request_id: str) -> Generator[str, None, None]:
        token_count = 0
        current_event = None
        last_len = 0
        try:
            for line in response.iter_lines():
                with self._requests_lock:
                    if request_id not in self._active_requests:
                        logger.info(f"🤖🗑️ Awlee stream {request_id} cancelled or finished externally during iteration.")
                        break
                ...
                if decoded.startswith("data:"):
                    ...
                    if current_event == "end":
                        logger.debug(f"🤖✅ [{request_id}] Awlee stream completed. End payload received.")
                        break
            logger.debug(f"🤖✅ [{request_id}] Finished yielding {token_count} Awlee tokens.")
        except Exception as e:
            is_cancelled = False
            with self._requests_lock:
                is_cancelled = request_id not in self._active_requests
            if is_cancelled:
                logger.warning(f"🤖⚠️ Awlee stream error likely due to cancellation for {request_id}: {e}")
            else:
                logger.error(f"🤖💥 Unexpected error during Awlee streaming ({request_id}): {e}", exc_info=True)
                raise ConnectionError(f"Awlee communication error during streaming: {e}") from e
        finally:
            if response and hasattr(response, 'close') and callable(response.close):
                try:
                    logger.debug(f"🤖🗑️ [{request_id}] Closing Awlee stream in _yield_awlee_chunks finally.")
                    response.close()
                except Exception as close_err:
                    logger.warning(f"🤖⚠️ [{request_id}] Error closing Awlee stream in finally: {close_err}", exc_info=False)
